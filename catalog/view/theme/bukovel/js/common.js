...
// Currency
$('.header__currency').change(function ()
	{
...
var cart = {
	...
	add: function (product_id, quantity, card_code) {
		...
		$.ajax({
			...
			if (json['success']) {
					...
					setTimeout(function () {
						//$('.cart-overlay').html('');
						//$('.header__cart').html('<span id="cart-total"><i class="fa fa-shopping-cart"></i> ' + json['total'] + '</span>');
					}, 1000);
				
					$('html, body').animate({ scrollTop: 0 }, 'slow');
					
					$('.cart-overlay').load('index.php?route=common/cart/info');
				} else {
					//console.log(json);
					return json;					
				}
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['cart_total']!= 0 ) {	
					$('.cart-overlay').load('index.php?route=common/cart/info');
					$($('.cart-data').find('.total-price').children()[1]).html(json['total_all']);
				} else {
