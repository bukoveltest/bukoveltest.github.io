...
$itemname = 'Пополнение клубных карт: катание';
$days = $_POST['days'];
$sum = $_POST['sum'];
$number = $_POST['number'];
...
$mail->Subject = 'Заявка';
$mail->Body    = $itemname . '<br>' .$number. 'шт. x '.$sum.'<br>'.$user_name.' '.$user_surname.'<br>'.'День рождения:'.$birthday.'<br> Пол:'.$sex.'<br>'.$country.' '.$phone.'<br>'.'Номер карты: '.$card.'<br>Заказ #'.$order;
$mail->AltBody = '';
